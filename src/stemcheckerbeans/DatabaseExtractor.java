...
//				String[] restrictions = new String[]{"Cell Proliferation", "Cell Cycle"};
				...
				ProteinList[] restrictioned = new ProteinList[0];
				
				if(restrictions.length>0)
				{
					//TODO: must hcange so that it only searches for restriced ones using the uploaded genes!
					
//					String queryStringRestrict = "select distinct aso.proteinList from Association as aso " +
//						"where aso.type in (:restrictions)";
					...
					*/
					...
					String queryStringRestrict = null;
					
					if(species==null)
					{
						queryStringRestrict = "select distinct aso.proteinList from Association as aso " +
							"where aso.type in (:restrictions) and aso.proteinList.entrezGeneId in (:genes)";
					}
					else
					{
						queryStringRestrict = "select distinct aso.proteinList from Association as aso " +
							"left join aso.proteinList.proteinAliases as alias " +
							"where aso.type in (:restrictions) and " +
							"(aso.proteinList.entrezGeneId in (:genes) or " +
							"aso.proteinList.geneSymbol in (:genes) or alias.alias in (:genes)) " +
							"and aso.proteinList.species = '"+species+"'";
					}
					
//					.setParameterList("genes", genes)
					
					Query queryObject = s.createQuery(queryStringRestrict).setParameterList("restrictions", restrictions).setParameterList("genes", genes);
					
					List l = queryObject.list();
					
					restrictedIds = new Integer[l.size()]; 
					
					restrictioned = new ProteinList[l.size()];
					
					for(int t=0;t<l.size();t++)
					{
						ProteinList restriProt = (ProteinList)l.get(t);
						
						restrictedIds[t] = restriProt.getId();
						
						restrictioned[t] = restriProt;
					}
...
//						System.out.println(found[a]+" THAT IS "+this.pvalues[a]);
...
//				tres[2] = this.overlaps[i];
				...
				checkForMatches(checkMatrix, secondCheckMatrix, matched1, matched2, aliasMatrix, genes, restrictioned, species!=null);
...
//		//System.out.println("----------");
	...
	protected void checkForMatches(CheckMatrix[] mat1, CheckMatrix[] mat2, boolean[] matched1, boolean[] matched2, ArrayList<String>[] aliasMatrix, String[] inputgenes, ProteinList[] restrictioned, boolean checkspecies)
	{
...
//					//System.out.println("b> "+inputgenes[z]);
				...
				{
					if(this.foundMatchInList(restrictioned, inputgenes[z], checkspecies)) masked.add(inputgenes[z]);
					else noMatches.add(inputgenes[z]);
...
//			//System.out.println("3> "+this.noSetMatches[i]);
		...
		this.masked = new String[masked.size()];
		
		for(int i=0;i<masked.size();i++)
		{
			this.masked[i] = masked.get(i);
//			//System.out.println("3> "+this.noSetMatches[i]);
		}
	}
...
//						title = ob.getPublicationTitle();
	...
	protected boolean foundMatchInList(ProteinList[] restrictioned, String id, boolean checkspecies)
	{
		boolean res = false;
		
		for(int i=0;i<restrictioned.length && !res;i++)
		{
			ProteinList pl = restrictioned[i];
			
			if(!checkspecies) res = pl.getEntrezGeneId().equals(id);
			else
			{
				res = pl.getEntrezGeneId().equals(id) || pl.getGeneSymbol().equals(id);
				
				if(!res)
				{
					Object[] aliases = pl.getProteinAliases().toArray();
					
					for(int t=0;t<aliases.length && !res;t++)
					{
						String alias = ((ProteinAlias)aliases[t]).getAlias();
						res = alias.equals(id);
					}		
